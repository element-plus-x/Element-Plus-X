...
Override `Conversations` `--elx-*` variables via `ConfigProvider.themeOverrides` (list background, item height, etc.).
</docs>
...
const items = ref<ConversationItem<{ id: string; label: string }>[]>([
  { id: '1', label: 'New chat', group: 'today' },
  { id: '2', label: 'Requirement', group: 'today' },
  { id: '3', label: 'Debugging', group: 'yesterday' },
  { id: '4', label: 'Release', group: 'last-week' }
]);
...
<template>
  <div style="display: flex; flex-direction: column; gap: 12px">
    ...
    >
      <div>Enable overrides to see background and item height changes.</div>
      <button
        type="button"
        style="
          padding: 6px 10px;
