...
import type { XFetchOptions } from './useXFetch'

import type { SSEOutput, XStreamOptions } from './useXStream'
import xFetch from './useXFetch'
...
class XRequestClass {
  ...
  private static instanceCacheMap: Map<string | typeof fetch, XRequestClass>
    = new Map()

  private constructor(options: XRequestOptions) {
    const { baseURL, model, dangerouslyApiKey, stream = true, ...customOptions } = options
  ...
  public static init(options: XRequestOptions): XRequestClass {
    ...
    if (!XRequestClass.instanceCacheMap.has(id)) {
      XRequestClass.instanceCacheMap.set(id, new XRequestClass(options))
    }

    return XRequestClass.instanceCacheMap.get(id) as XRequestClass
  }
